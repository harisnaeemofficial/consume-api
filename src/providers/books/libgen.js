...
import { LibgenBookObject } from "../../models";
...
class Libgen {
  ...
  Book = async (bookUrl) => {
    ...
    container.link = `${this.downloadIP}/main/${floorID(
      ...
      } ${rawAuthor} - ${container.title}-${container.publisher} (${
        container.year
      }).${container.format}`
    )}`;
    return container;
  ...
  Search = async (query, page = 1) => {
    ...
    for (let i = 0; i < containers.length; i++) {
      ...
      const data = await axios.get(containers[i].link);
      ...
      containers[i].link = `${this.downloadIP}/main/${floorID(
        ...
        `${
          containers[i].series == "" ? "" : `(${containers[i].series})`
        } ${rawAuthor} - ${containers[i].title}-${containers[i].publisher} (${
          containers[i].year
        }).${containers[i].format}`
      )}`;
    }
