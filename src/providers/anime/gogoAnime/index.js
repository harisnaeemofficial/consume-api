...
class Gogoanime {
  ...
  Source = async (episodeId, server = StreamingServers.GogoCDN) => {
    ...
    try {
      ...
      return await this.Source(serverUrl.href, server);
    } catch (err) {
      console.error(err);
  ...
  RecentEpisodes = async (page = 1, type = 1) => {
    try {
      const res = await axios.get(
