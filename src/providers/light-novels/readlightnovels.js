...
class ReadLightNovels {
  name = "Read Light Novels";
  baseUrl = "https://readlightnovels.net";

  logo = "https://i.imgur.com/RDPjbc6.png";
  classPath = "LIGHT_NOVELS.ReadLightNovels";

  Info = async (lightNovelUrl, chapterPage = -1) => {
    ...
    try {
      ...
      if (chapterPage === -1) {
        lightNovelInfo.chapters = await this.AllChapters(
          novelId,
          pages,
          lightNovelUrl
        );
      } else {
        lightNovelInfo.chapters = await this.Chapters(
          novelId,
          chapterPage,
          lightNovelUrl
        );
  ...
  Chapters = async (novelId, chapterPage, referer) => {
    const chapters = [];
  ...
  AllChapters = async (novelId, pages, referer) => {
    const chapters = [];

    for (const pageNumber of Array.from({ length: pages }, (_, i) => i + 1)) {
      const chaptersPage = await this.Chapters(novelId, pageNumber, referer);
      chapters.push(...chaptersPage);
    }
    return chapters;
  };

  ChapterContent = async (chapterId) => {
